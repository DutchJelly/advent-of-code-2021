...
input2.forEach(line => {
    ...
    const remap = [6, 5, 4, 3, 2, 1, 0];

    const firstPart = line.split(' | ')[0];

    //a1 n a2 === a1 u a2
    const similarArray = (a1, a2) => {
        if (a1.length !== a2.length) return false;
        let a2copy = [...a2];
        for (let i = 0; i < a1.length; i++) {
            let matched = false;
            for (let j = 0; j < a2copy.length && !matched; j++) {
                if (a1[i] === a2copy[j]) {
                    a2copy[j] = undefined;
                    matched = true;
                }
    ...
    const getDigit = (letters) => {
        return digitMap.findIndex(val => similarArray(val, [...letters]));
    }

    const rewireString = (str) => {
        return [...str].map(x => wires[remap[wires.indexOf(x)]])
    }

    const isValid = () => {
        let allDigits = true;
        firstPart.split(' ').forEach(x => {
            if (getDigit(rewireString(x)) === -1) {
                allDigits = false;
                return;
            }
        });
    ...
    //Increase remap by 1 (passes all combinations).
    const increase = (i = 0) => {
        if (i === 7) {
            return false;
        }
    ...
    while (increase()) {
        if (uniqueElemArray(remap) && isValid())
            break;
    }
    let num = 0;
    line.split(' | ')[1].split(' ').forEach(x => {
        num *= 10;
        num += getDigit(rewireString(x));
    });
    total += num;
});
