const input2 = ['beacf afbd bcead cgefa ecdbga efb gbfdeac ecgfbd acbdfe fb | bf efb bgecdfa egcfa',
...
input.forEach(line => {
    //hou bij van elke letter waar die allemaal kan zitten
    //zoek voor intersections

    const rewiremap = {};


    const analysis = line.split(' | ')[0].split(' ');
    analysis.forEach(anal => {
        const matchingLengths = digitMap.filter(x => {
            return x.length == anal.length;
        });


        [...anal].forEach(ch => {
            if (!rewiremap[ch]) rewiremap[ch] = [];
            rewiremap[ch].push(matchingLengths);
        });
    });

    //find the mapping that matches them all
    //fuck brute force :)

    const parsedRewireMap = {};
    Object.keys(rewiremap).forEach((ch) => {
        const mapping = rewiremap[ch];
        const possibleChars = [];
        ['a', 'b', 'c', 'd', 'e', 'f', 'g'].forEach(possible => {
            if (!mapping.find(x => !x.find(y => y.find(z => z === possible))))
                possibleChars.push(possible);
        });
        parsedRewireMap[ch] = possibleChars;
    });

    const wires = ['a', 'b', 'c', 'd', 'e', 'f', 'g'];
    const remap = [0, 0, 0, 0, 0, 0, 0];
    //now brute force the last bit
    ...
    const isValid = (i) => {
        if (!parsedRewireMap[wires[i]]) return true;
        return parsedRewireMap[wires[i]].find(x => x === wires[remap[i]])
    }

    while (increase(0)) {

        let allUnique = true;
        for (let i = 0; i < 7 && allUnique; i++) {
            for (let j = i + 1; j < 8 && allUnique; j++) {
                if (remap[i] === remap[j])
                    allUnique = false;
            }
        }
        ...
        if (valid) {
            remap.forEach((x, index) => {
                parsedRewireMap[wires[index]] = wires[x];
            });
            break;
            // console.log(
        }
    ...
    let number = 0;
    console.log(parsedRewireMap);

    line.split(' | ')[1].split(' ').forEach(x => {
        console.log(x);
        console.log(Object.keys(digitMap).find(y => {
            return x.length === digitMap[y].length && ![...x].find(z => !digitMap[y].includes(parsedRewireMap[z]));
        }));
    })
});
