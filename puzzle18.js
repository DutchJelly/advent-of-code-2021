const input = [[9, 9, 8, 7, 6, 5, 3, 2, 3, 4, 5, 8, 9, 6, 5, 4, 5, 4, 9, 8, 6, 5, 4, 3, 2, 1, 0, 1, 2, 3, 5, 7, 8, 9, 9, 7, 9, 7, 6, 5, 4, 5, 9, 8, 3, 2, 1, 0, 1, 2, 3, 4, 5, 6, 7, 9, 4, 3, 9, 8, 6, 5, 4, 3, 1, 0, 1, 2, 3, 9, 8, 7, 4, 3, 2, 1, 0, 1, 2, 9, 3, 2, 3, 9, 7, 3, 1, 9, 9, 8, 5, 2, 1, 4, 8, 9, 6, 5, 4, 3,],
...
//made with regex magic
const input2 = [[2, 1, 9, 9, 9, 4, 3, 2, 1, 0,],
[3, 9, 8, 7, 8, 9, 4, 9, 2, 1,],
[9, 8, 5, 6, 7, 8, 9, 8, 9, 2,],
[8, 7, 6, 7, 8, 9, 6, 7, 8, 9,],
[9, 8, 9, 9, 9, 6, 5, 6, 7, 8,],]
...
const getNeighbors = (i, j) => {
    ...
    for (let ii = i - 1; ii < i + 2; ii++) {
        for (let jj = j - 1; jj < j + 2; jj++) {
            ...
            if (ii >= input.length || jj >= input[0].length) continue;

            //No diagonals :(
            if (Math.abs(ii - i) === 1 && Math.abs(jj - j) === 1) continue;
...
const includesElem = (a, elem) => {
    return !!a.find(x => arraysMatch(x, elem));
}

let basins = [];

const flow = (visited, called = 0) => {
    let continueFlow = false;
    visited.forEach(point => {
        let nb = getNeighbors(point[0], point[1]);
        nb = nb.filter(x => !includesElem(visited, x) && input[x[0]][x[1]] < 9);
        if (nb.length > 0) {
            continueFlow = true;
            visited.push(...nb);
        }
    })
    if (continueFlow) return flow(visited, called + 1);
    else return visited.length;
}

for (let i = 0; i < input.length; i++) {
    for (let j = 0; j < input[i].length; j++) {
        const nb = getNeighbors(i, j);
        const current = input[i][j];

        if (nb.find(x => input[x[0]][x[1]] <= current) !== undefined) continue;

        basins.push(flow([[i, j]]));
...
basins.sort((a, b) => b - a);
console.log(`the answer: ${basins[0] * basins[1] * basins[2]}`);