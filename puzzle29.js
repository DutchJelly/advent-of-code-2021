...
const getLowestUnvisited = () => {
    ...
    Object.keys(nodes).forEach(x => {
        if (nodes[x].visited) return;
        if (!lowest) {
            lowest = nodes[x];
            lowestId = x;
            return;
        }
        if (nodes[x].shortest < lowest.shortest) {
            lowest = nodes[x];
            lowestId = x;
        }
    })
...
let [lowestId, lowest] = getLowestUnvisited();
while (lowest) {
    ...
    neighBours.forEach(([ni, nj]) => {
        ...
        if (lowest.shortest + nodes[nId].weight < nodes[nId].shortest) {
            nodes[nId].shortest = lowest.shortest + nodes[nId].weight;
            nodes[nId].previous = lowestId;
        }
    });

    lowest.visited = true;
    [lowestId, lowest] = getLowestUnvisited();
}

//Added -1 which for some reason was the right answer... Dijkstra is greedy after all I guess
console.log(`Risk level: ${nodes[getIJIdenifier(input.length - 1, input[0].length - 1)].shortest - 1}`);
