const puzzleInput = [
    [2, 3, 3, 1, 3, 8, 8, 3, 2, 3, 6, 3, 8, 1, 6, 1, 7, 8, 7, 3, 9, 9, 3, 9, 1, 2, 6, 3, 5, 6, 9, 1, 2, 3, 1, 1, 5, 2, 9, 2, 6, 1, 8, 3, 4, 2, 2, 2, 7, 1, 4, 1, 2, 9, 5, 8, 2, 6, 6, 3, 1, 1, 8, 2, 1, 9, 1, 1, 2, 9, 4, 2, 6, 2, 5, 3, 1, 1, 8, 1, 7, 1, 4, 3, 6, 2, 3, 8, 1, 1, 5, 1, 1, 1, 1, 2, 4, 3, 6, 2,],
...
const tileSizeX = input[0].length;
const tileSizeY = input.length;
let newInput = [];
newInput = new Array(tileSizeY * 5);
for (let i = 0; i < tileSizeY * 5; i++) {
    newInput[i] = new Array(tileSizeX * 5);
    for (let j = 0; j < tileSizeX * 5; j++) {
        newInput[i][j] = (input[i % tileSizeY][j % tileSizeX] + Math.floor(i / tileSizeY) + Math.floor(j / tileSizeX)) % 9;
        if (newInput[i][j] === 0) newInput[i][j] = 9;
    }
}
input = newInput;

const shortests = new Array(input.length * input[0].length); //array of shortest values for each node
const visited = new Array(input.length * input[0].length); //array of unvisited node id's

const getIJIdenifier = (i, j) => i * input[0].length + j;
const getIJFromIdentifier = (id) => [Math.floor(id / input[0].length), id % input[0].length];

const getNeighbours = (i, j) => [
    i === 0 ? undefined : [i - 1, j],
    i === input.length - 1 ? undefined : [i + 1, j],
    j === 0 ? undefined : [i, j - 1],
    i === input[0].length - 1 ? undefined : [i, j + 1]
...
shortests.fill(Number.MAX_VALUE);
shortests[0] = 0;

let lowestId = getLowestUnvisited();
while (lowestId !== undefined) {
    visited[lowestId] = true;
    const [i, j] = getIJFromIdentifier(lowestId);
    const neighBours = getNeighbours(i, j);

    neighBours.forEach(([ni, nj]) => {
        nId = getIJIdenifier(ni, nj);
        if (input[ni][nj] + shortests[lowestId] < shortests[nId]) {
            shortests[nId] = input[ni][nj] + shortests[lowestId];
        }
    });
    lowestId = getLowestUnvisited();
}

//It does take a while to run..
//To reduce this time I could look into the effect of longer arrays instead of 2d ones, look into
//efficiency of global function declarations or use some other library instead of Array.
console.log(`Risk level: ${shortests[getIJIdenifier(input.length - 1, input[0].length - 1)]}`);