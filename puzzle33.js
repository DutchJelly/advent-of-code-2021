...
const shoot = (probe, maxYRef = {}) => {
    ...
    if (underShotTarget(probe)) {
        if (probe.velx === 0 && probe.x > input.xmax && probe.x < input.xmin)
            return 'badundershot'
        return 'undershot';
    }
    if (overShotTarget(probe)) return 'overshot';
    return 'hit';
}
...
for (probe.velx = 1; probe.velx <= input.xmax; probe.velx++) {
    ...
    for (probe.vely = input.ymin; probe.vely < 7000; probe.vely++) {
        const shootresult = shoot({ ...probe });
        if (shootresult === 'hit') {
            if (!bestProbe || bestProbe.vely < probe.vely) bestProbe = { ...probe };
        }
        if (shootresult === 'overshot' || shootresult === 'badundershot') break;
    }
}

let maxYRef = {};
shoot(bestProbe, maxYRef);

console.log(maxYRef.value);
