...
function reduceTree(root) {
    ...
    // console.log('int ' + JSON.stringify(toArray(root)));
}
...
const trueRoot = parseInput(input);

let lastRoot = parseInput(input[0]);
for (let i = 1; i < input.length; i++) {
    console.log('   ' + JSON.stringify(toArray(lastRoot)));

    let currentRoot = parseInput(input[i]);
    console.log('+  ' + JSON.stringify(toArray(currentRoot)));
    //combine last and current
    let newRoot = {
        children: [
            lastRoot,
            currentRoot,
    ...
    lastRoot.parent = newRoot;
    currentRoot.parent = newRoot;
    reduceTree(newRoot);
    lastRoot = newRoot;
    console.log('=  ' + JSON.stringify(toArray(lastRoot)));
}
console.log(calcMagnitude(lastRoot));